	...
	<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
		<div class="kt-portlet kt-portlet--mobile">
				...
				<div class="kt-portlet__head-toolbar">
					<div class="kt-portlet__head-wrapper">
						<div class="kt-portlet__head-actions">
							...
							&nbsp;
							<a href="<?php echo base_url('admin/tournaments/add_tournament') ?>"
			...
			<div class="kt-portlet__body">
				...
				<!--begin: Datatable -->
				<table class="table table-striped table-bordered table-hover table-checkable  text-justify text-center" id="table_tournaments">
					<thead>
					<tr>
						<th>#</th>
						<th>Logo</th>
...
<!--							<td>--><?php //echo date_format(date_create($tournament->squad_submission_date), 'M d, Y'); ?><!--</td>-->
							<td>
								<?php if ($tournament->status == '1') { ?>
									<span class="kt-badge  kt-badge--success kt-badge--inline kt-badge--pill"
											onclick="change_status(<?php echo $tournament->id ?>,<?php echo $tournament->status ?>)">
										Active
									</span>
								<?php } else { ?>
...
<!--									   data-toggle="kt-tooltip" data-placement="top" title="Invite Teams">-->
<!--										<i class="la la-users"></i></a>-->
<!--									<a href="--><?php //echo base_url('admin/tournaments/confirmed_team_list/' . $tournament->id) ?><!--"-->
<!--									   class="btn btn-outline-success btn-elevate btn-elevate-air btn-icon btn-sm" data-container="body"-->
<!--									   data-toggle="kt-tooltip" data-placement="top" title="Confirmed Teams List">-->
<!--										<i class="la la-users"></i></a>-->
...
<!--									-->
									<a href="<?php echo base_url('admin/tournaments/manage_tournament/' . 'home/' . $tournament->id) ?>"
									   class="btn btn-outline-success btn-elevate btn-elevate-air btn-icon btn-sm" data-container="body"
									   data-toggle="kt-tooltip" data-placement="top" title="View">
...
<script>
	...
	function change_status(id, status) {
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url("admin/tournaments/change_status");?>',
	...
	function change_tournament_id(id) {
		$('#tournament_id').val(id);
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url("admin/tournaments/get_teams_by_tournament_id");?>',
			dataType: "json",
			data: {id:id},
			success: function (result) {
				if ($.trim(result)) {
					...
					for (i = 0; i < result.length; i++) {
						html += '<option value=' + result[i].id + '>' + result[i].company_name + '</option>';
					}
					$('#first_team').html(html);
					$('#second_team').html(html);
				}
				else{
					$('#first_team').empty()
					$('#second_team').empty();

					$('#first_team').html('<option value="">No Teams Found</option>');
					$('#second_team').html('<option value="">No Teams Found</option>');
				}
			}
		});
	}
