	...
	<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
		<div class="kt-portlet kt-portlet--mobile">
			...
			<div class="kt-portlet__body">
				...
				<!--begin: Datatable -->
				<table class="table table-striped table-bordered table-hover table-checkable" id="team_tournament_table">
					<thead>
					<tr>
						<th>#</th>
					...
					foreach ($tournaments as $tournament) { ?>
						<tr>
							...
							<td>
								<?php if ($tournament->tournament_status == '1') { ?>
									<span class="kt-badge  kt-badge--success kt-badge--inline kt-badge--pill">Live</span>
								<?php } else if ($tournament->tournament_status == '2'){ ?>
									<span class="kt-badge  kt-badge--danger kt-badge--inline kt-badge--pill">Finished</span>
								<?php } else { ?>
								<span class="kt-badge  kt-badge--success kt-badge--inline kt-badge--pill">Pending</span>
								<?php } ?>
							</td>

							<td>
								<?php
								$date = new DateTime($tournament->squad_submission_date);
								$now = new DateTime();
								if ($tournament->squad_submission_date >= date('Y-m-d')) { ?>
									<?php if ($tournament->mapping_status == '2') { ?>
											<a href="<?php echo base_url('team/squad/submit_squad/' . $tournament->id) ?>"
											   class="btn btn-outline-primary btn-elevate btn-elevate-air btn-icon btn-sm" data-container="body"
											   data-toggle="kt-tooltip" data-placement="top" title="Submit Squad">
												<i class="fa fa-users"></i></a>
										</td>
									<?php } else if ($tournament->mapping_status == '0') { ?>
										<td>
											<a href="<?php echo base_url('invitation/invitation/' . $tournament->invitation_id) ?>"
											   target="_blank" class="btn btn-outline-primary btn-elevate btn-elevate-air btn-icon btn-sm"
											   data-container="body"
											   data-toggle="kt-tooltip" data-placement="top" title="Accept Invite">
...
<script>
	$('document').ready(function () {
		$('#team_tournament_table').DataTable({
			responsive: true,
			// Pagination settings
